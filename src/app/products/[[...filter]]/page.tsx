...
async function getProducts(filters: any) {
  ...
  try {
    ...
    console.error(
      "Failed to fetch products in route page during build:",
      error
    );
...
export default async function ProductsPage({
  ...
  if (filter && filter.length > 0) {
    ...
    if (filter.length === 1) {
      ...
      if (type === "price") {
        ...
        } else if (slug === "2000-4999") {
          minPrice = 2000;
          maxPrice = 4999;

        } else if (slug === "5000-9999") {
          minPrice = 5000;
          maxPrice = 9999;
  ...
  if (bond) {
    displayTitle =
      `Gifts for ${bond.replace("-", " ")}`;
  }

  if (tag) {

    if (tag === "for-her") {
      displayTitle = "Gifts for Her";

    } else if (tag === "for-him") {
      displayTitle = "Gifts for Him";

    } else if (tag === "gift-sets") {
      displayTitle = "Exclusive Gift Sets";

    } else {
      displayTitle =
        `${tag.replace("-", " ").charAt(0).toUpperCase()}${tag.replace("-", " ").slice(1)} Collection`;
    }
  }

  if (search) {
    displayTitle = `Results for "${search}"`;
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        ...
        <div>
          ...
          <p className="text-gray-500 font-medium">
            Discover your perfect piece of elegance
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            <Link
              ...
              className="block text-sm font-medium text-gray-400 hover:text-primary"
            >
              Clear Category
            </Link>
          </div>
          ...
          products.map((product: any) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <h2 className="text-lg font-bold text-secondary group-hover:text-primary transition-colors line-clamp-1 mb-2">
                  {product.name}
                </h2>
