      ...
      try {
        ...
        if (product.collection?.slug) {
          const res = await fetch(
            `/api/products?occasion=${encodeURIComponent(
              product.collection.slug
            )}&excludeId=${product.id}&limit=3`
          );
          ...
          if (res.ok) {
            ...
            // FIX
            const collectionItems = Array.isArray(response)
              ? response
              : response.products || [];
        ...
        if (items.length < 2) {
          const res = await fetch(
            `/api/products?excludeId=${product.id}&limit=4`
          );
          ...
          if (res.ok) {
            ...
            // FIX
            const generalItems = Array.isArray(response)
              ? response
              : response.products || [];
        ...
        console.error(
          "Failed to fetch bundle suggestions:",
          error
        );
