...
export default function SimilarProducts({ categoryId, categorySlug, excludeId }: Props) {
  ...
  useEffect(() => {
    async function fetchSimilar() {
      try {
        ...
        // 1. Fetch matching category products
        if (categorySlug) {
          const url = `/api/products?category=${encodeURIComponent(categorySlug)}&excludeId=${excludeId}&limit=4`;
        ...
        // 2. Fallback: if we have fewer than 4 products in this category, fetch generic active items to populate the remaining slots
        if (data.length < 4) {
          const fallbackRes = await fetch(`/api/products?excludeId=${excludeId}&limit=8`);
          if (fallbackRes.ok) {
            const fallbackData = await fallbackRes.json();
            const existingIds = new Set(data.map((p) => p.id));
            for (const item of fallbackData) {
              if (!existingIds.has(item.id) && data.length < 4) {
                data.push(item);
              }
            }
          }
      ...
      <div className="mt-24 border-t border-gray-100 pt-16">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <h2 className="text-3xl font-bold text-secondary uppercase tracking-tight">Similar Masterpieces</h2>
            <div className="h-1 w-20 bg-primary rounded-full mt-2" />
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="animate-pulse space-y-4 bg-white p-4 border border-gray-50 rounded-2xl">
